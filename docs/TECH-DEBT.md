...
| **13** | `Custódia hardware-backed das chaves derivadas (Keystore/Keychain)` | `KeyVault` (Android/iOS/Desktop) | **Médio (Segurança em Profundidade)**. Atualmente, a derivação da semente mnemônica de 12 palavras para pares Ed25519/X25519 ocorre em software via Argon2id + Ed25519 pure-Kotlin. A custódia em repouso reside no dispositivo sob proteção do sistema operacional (DPAPI no Windows, Keystore no Android). Chaves derivadas Ed25519 não são suportadas diretamente pelo Secure Enclave da Apple nem por versões legadas do StrongBox. | Avaliar encapsulamento da semente mnemônica e das chaves derivadas sob chave-mestre assimétrica de hardware (Hardware KeyStore/Keychain) no ciclo v1.6/v2.0. | Backlog v1.6/v2.0 |
